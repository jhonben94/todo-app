...
(() => {
    ...
    document.onreadystatechange = () => {
        ...
        Ajax.sendGetRequest(API_URL,{}, MediaFormat.JSON,
         (callbackSuccess) => {
                             loadTasks(callbackSuccess);
                            },
        (callbackError) => {
                             showError(callbackError,'No se obtuvieron los datos necesarios. ');
                        }, 
        true
        );
    ...
    const showError = (code, text) => {
        ...
        element.innerHTML="Ha ocurrido un error: "+ text + " Codigo: "+code;
    ...
    const showWarning = (code, id) => {
        var element = document.getElementById("warningDiv");
        element.style.display="block";
        element.innerHTML="La tarea se modifico correctamente la tarea con id:"+ id;
        setTimeout(()=>{
            element.style.display="none";
        }, 3000);
    ...
    const addTask = (e) => {
        ...
        Ajax.sendPostRequest(API_URL,task, MediaFormat.JSON,
            (callbackSuccess) => {
                console.log(callbackSuccess);
                                addTaskToList(task);
                                newTaskInput.value='';
                                showSuccess(callbackSuccess,'Se agrego correctamente la tarea.');
                               },
           (callbackError) => {
                                showError(callbackError,'No se obtuvieron los datos necesarios. ');
                           }, 
    ...
    const revertHTMLChangeOnEdit = (currentTask) => {
        let task = currentTask;//JSON.parse(currentTask);
        ...
        Ajax.sendDeleteRequest(API_URL+`/${id}`,{}, MediaFormat.JSON,
            (callbackSuccess) => {
                                removeTaskFromList(id);
                                 showSuccess(200,'Se elimino correctamente la tarea.')
                               },
           (callbackError) => {
                                showError(callbackError,'No se pudo modificar la tarea.');
                           }, 
